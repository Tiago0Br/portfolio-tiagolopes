...
export function Projects({ projects }: ProjectProps) {
    return (
        <article
            className='space-y-10 flex flex-col items-center xl:items-start text-center xl:text-left'
        >
                ...
                {projects.map(({ name, slug, image }, index) => (
                    <Link href={slug} key={name + index} target='_blank'>
                        <li className='text-md relative'>
                            ...
                            <span>{name}</span>
                            <div
                                className='bg-t-blue-500 rounded-xl w-14 h-14 text-center
                                flex justify-center items-center text-3xl absolute bottom-[1.25rem]
                                -right-[1.25rem]'
                            >
                                <span>{index + 1}</span>
                            </div>
